...
class CVAE(nn.Module):
    def __init__(self, src_vocab_size, trg_vocab_size, embed_size, hidden_size, latent_size, num_layers, dpt=0.3, word_dpt=0.0, share_params=False):
...
#         self.p = inferer.SelfAttentionPrior(hidden_size, latent_size, dpt)
    ...
    def forward(self, src, trg):
        if self.share_params:
            encoded_src_t = encoded_src_i = self.shared_encoder(self.src_embedding(src))
